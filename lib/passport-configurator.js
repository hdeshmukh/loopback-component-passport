...
PassportConfigurator.prototype.configureProvider = function (name, options) {
                  ...
                  self.userModel.login(creds,
                    function (err, accessToken) {
                      ...
                      if (accessToken && user.emailVerified) {
                        ...
                        if (!accessToken) {
                          err = new Error(g.f('Failed to create token.'));
                        } else if (!user.emailVerified) {
                          err = new Error(g.f('User has not verified their email address'));
                        } else {
                          err = new Error();
                        }

                        done(err, userProfile, accessToken);
                      }
                    });
                };
              } else {
   ...
   */
  if (authType === 'local') {
    if (options.forceDefaultCallback) {
      self.app.post(authPath, defaultCallback);
    } else {
