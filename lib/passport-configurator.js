...
PassportConfigurator.prototype.configureProvider = function(name, options) {
  ...
  switch (authType) {
    case 'ldap':
      passport.use(provider, new AuthStrategy(_.defaults({
              ...
              if (user) {
                ...
                if (options.setAccessToken) {
                  ...
                  function login(creds) {
                    var ttl = lookup(req.body, 'ttl') || lookup(req.query, 'ttl');
                    creds.ttl = ttl;
  ...
  var defaultCallback = function (req, res, next) {
    ...
    passport.authenticate(provider, _.defaults({session: session},
  ...
  if (authType === 'local' || authType === 'ldap'){
    ...
    self.app.get(authPath, passport.authorize(provider, _.defaults({
      scope: scope,
      session: session
    }, options.authOptions)));
  } else {
    self.app.get(authPath, passport.authenticate(provider, _.defaults({
      scope: scope,
      session: session
    }, options.authOptions)));
  }
   ...
   */
  if (link) {
    self.app.get(callbackPath, passport.authorize(provider, _.defaults({
          session: session,
